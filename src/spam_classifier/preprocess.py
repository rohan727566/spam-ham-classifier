...
def preprocess_texts(texts: List[str], verbose: bool = True) -> List[str]:
    ...
    if verbose:
        ...
        print(f"  Average length before: {sum(len(t.split()) for t in texts) / len(texts):.1f} words")
        print(f"  Average length after: {sum(len(t.split()) for t in cleaned_texts) / len(cleaned_texts):.1f} words")
